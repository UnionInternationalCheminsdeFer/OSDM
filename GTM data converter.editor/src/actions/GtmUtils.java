...
public class GtmUtils {
	...
	public static CompoundCommand getPreparationCommand (GTMTool tool, EditingDomain domain) {
		...
		if (tool.getConversionFromLegacy() == null) {
			...
			if (tool.getConversionFromLegacy().getParams() == null) {
				...
				if (tool.getConversionFromLegacy().getParams().getLegacyZoneMappings() == null) {
					command.append(new SetCommand(domain,tool.getConversionFromLegacy().getParams(),GtmPackage.Literals.CONVERSION_PARAMS__LEGACY_ZONE_MAPPINGS,GtmFactory.eINSTANCE.createLegacyZoneMappings()));									
				}
	...
	public static CompoundCommand setIds(GTMTool tool, EditingDomain domain) {
		...
		i = 0;
		for (ZoneDefinition object : fareStructure.getZoneDefinitions().getZoneDefinitions()) {
			i++;
			if (object.getId() == null || object.getId().isEmpty()) {
				setId(domain, object,GtmPackage.Literals.ZONE_DEFINITION__ID, command, listName,i);
			}
		}						
