...
Some OSDM requests potentially support a large number of parameters (e.g. filters).
Supporting all of them is not always possible, e.g. due restrictions of the
underlying systems. Thus the following rule applies:

- If the parameter is required, return an _error_ "NOT SUPPORTED".
- If the parameter is optional, return a _response_ including a _warning_ indicating
  that the optional request parameter was ignored.
